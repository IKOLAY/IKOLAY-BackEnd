...
public class CompanyService extends ServiceManager<Company, Long> {
    ...
    private void addDefaultCompany() {
        ...
        if (company.isEmpty()) {
            save(Company.builder().companyName("DUMMY CORP.").logo("https://bcassetcdn.com/social/o5jw7zottz/preview.png").phone("7343971529").address("29488 Brown Ct Garden City Michigan 48135 USA").about("Managed Analyzing Interface").taxNo("7427354101").build());
            save(Company.builder().companyName("BİLGE ADAM").logo("https://cdn.cloudwises.com/ba-assets/genel/icon.svg").phone("8502016000").address("Reşitpaşa Mah. Katar Cad. İTÜ Teknokent Arı 3 No:4 B3 Sarıyer / İSTANBUL").about("BilgeAdam Teknoloji; 1997’de İstanbul’da kurulmuş, global operasyonlarını İngiltere ve Avrupa üzerinden yaygınlaştıran bir yazılım şirketidir.").taxNo("5344596291").build());
            save(Company.builder().companyName("TOKİNAN INC.").logo("https://bcassetcdn.com/social/chm5gip2m2/preview.png").phone("2129951107").address("53 E 8th St Manhattan North Dakota 10003 USA").about("Visionary Zeroadministration Database").taxNo("1335114731").build());
            save(Company.builder().companyName("GÜLNİHAL INC.").logo("https://bcassetcdn.com/social/h9qi7fxc4w/preview.png").phone("8136851909").address("333 N Falkenburg Rd Tampa Florida 33619 USA").about("Realigned Background Firmware").taxNo("3361079111").build());
            save(Company.builder().companyName("MARTLI INC.").logo("https://bcassetcdn.com/social/fg1coht6y0/preview.png").phone("7342130017").address("170 Enterprise Dr Ann Arbor Michigan 48103 USA").about("Cloned Interactive Synergy").taxNo("0709367041").build());
            save(Company.builder().companyName("ADANEDHEL INC.").logo("https://bcassetcdn.com/social/ybmnhv0fs6/preview.png").phone("5128649911").address("2425 Williams Dr Georgetown Texas 78633 USA").about("Self-Enabling Contextually-Based Encoding").taxNo("0676243201").build());
        }
    }
