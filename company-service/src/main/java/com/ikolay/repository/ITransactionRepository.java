...
public interface ITransactionRepository extends JpaRepository<FinancialTransaction, Long> {
    ...
    List<FinancialTransaction> findAllByTransactionDateBetweenAndIsPaidAndTypeAndCompanyId(LocalDate start, LocalDate end, Boolean isPaid, ETransactionType type,Long companyId);


    @Query("Select new com.ikolay.dto.response.AnnualProfitLossResponseDto(YEAR(f.transactionDate) as year, SUM(f.transactionAmount) as yearsTotal) from FinancialTransaction f where f.companyId = ?1 and f.isPaid=true and f.transactionDate Between ?2 and ?3 group by year(f.transactionDate)")
